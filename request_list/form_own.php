...
?>
    ...
    <p style="padding:3px;"><label style="vertical-align:top; width:80px;display:inline-block;"> รายละเอียด <?php echo $i ?></label><textarea  name="detail<?php echo trim($i) ?>" 
...
<tr>
  ...
  <td><?php echo ($rs['orderid']==0)?GetUser($_SESSION['id'],'user'):GetUser($rs['orderid'],'user'); ?></td>
</tr>
<tr>
  <th valign="top">รายละเอียดการส่งต่อ</th>
  <td><textarea name="send_note" cols="72" rows="5" id="textarea5" disabled="disabled"><?php echo $rs['send_note'];?></textarea></td>
</tr>
<tr>
  <th valign="top">เจ้าของระบบ</th>
  <td>
  <?php if($rs['chk_send']=="send" || $rs['chk_send']=="send_wait"){ ?>
  <input type="checkbox" name="system_success" value="1" <?php  echo($rs['system_success']=="1")? "checked":"";?>/> เสร็จ</td>
  <?php }else { ?>
...
<tr>
  ...
  <td><input type="checkbox" name="response_success"value="1" <?php  echo($rs['response_success']=="1")? "checked":"";?> disabled="disabled" /> เสร็จ</td>
</tr>
<tr>
  ...
  <?php if($rs['chk_send']=="send" || $rs['chk_send']=="send_wait"): ?>
	  <?php if($rs['ownid']==0 || $rs['ownid']=="") {  ?>
        <input type="hidden" name="ownid"				value="<?php echo $_SESSION['id']?>" />
      <?php }else{ ?>
