...
function form_disable()
{
	...
	$("input[name='service']:checked").attr('disabled','disabled');
	$("input[name='service']").attr('disabled','disabled');
  ...
  $("#n_orderid").attr('disabled','disabled');
...
function form_enable(){
	...
	$("input[name='service']:checked").removeAttr('disabled');
	$("input[name='service']").removeAttr('disabled');
	 ...
	 $.validator.setDefaults({
	  ...
	  $("#frmedit").validate({
		...
		messages: {
		  ...
		  detail: "  กรุณาพิมพ์รายละเอียดปัญหา",
...
<tr>
  ...
  <?php if ($rs['orderid']==0 || $rs['orderid']==""): ?>
        <input name="n_orderid" type="text" id="n_orderid" size="40" onclick='$("a[name=orderid]").trigger("click");' />
         <a name="order" class="a_search" href="request_list/order_search.php?total=total"></a>
...
<tr>
	...
	<?php if($rs['coordinatorid']!=0):?>
  		<?php echo $coor=GetUser($rs['coordinatorid'],'coor');?>
        <input name="coordinatorid" type="hidden" value="<?php echo $rs['coordinatorid'];?>" />
    <?php endif;?>          
    <?php if($rs['coordinatorid']==0 ): ?>
    	<?php echo $coor=GetUser($_SESSION['id'],'user');?>
        <input name="coordinatorid" type="hidden" value="<?php echo $_SESSION['id'];?>" />
 ...
 <input type="hidden" name="id" 				value="<?php echo $rs['id']; ?>" />
  ...
  <input type="hidden" name="ownid"  			value="<?php echo $rs['ownid'] ?>" />
   ...
   <?php if($rs['status']==""):?>
	...
	<input type="hidden" name="coordinatorid"	value="<?php echo $_SESSION["id"]?>" />
  <?php endif; ?>
  <?php if($rs['status']!=""):?>
  	<input type="hidden" name="new_date"		value="<?php echo $rs['new_date'];?>" />
  	<input type="hidden" name="complete_date" 	value="<?php echo $rs['complete_date']?>" />
